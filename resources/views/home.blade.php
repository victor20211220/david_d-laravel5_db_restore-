...
@section('page_title')
    New Lead <small>List a new lead.</small>
@endsection
            ...
            <!-- form start -->
                <div class="box-body no-padding">
                    ...
                    <div class="box-group" id="accordion">
                        ...
                        <div class="panel box box-default box-flat">
                            <div class="box-header with-border">
                                <h4 class="box-title">
                                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" onclick="changeAcc('call');" aria-expanded="true" class="" data-active="true">
                                        Call Lead
                                    </a>
                                </h4>
                            </div>
                            <div id="collapseOne" class="panel-collapse collapse in" aria-expanded="true">
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        {{ Form::text('lead_name', Input::old('lead_name'), array('class' => 'form-control', 'placeholder'=>'Full Name')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('lead_email', 'Email Address') }}
                                        {{ Form::text('lead_email', Input::old('lead_email'), array('class' => 'form-control', 'placeholder'=>'Email Address')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('lead_email_alt', 'Alternative Email Address') }}
                                        {{ Form::text('lead_email_alt', Input::old('lead_email_alt'), array('class' => 'form-control', 'placeholder'=>'Alternative Email')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('lead_phone', 'Phone Number') }}
                                        {{ Form::text('lead_phone', Input::old('lead_phone'), array('class' => 'form-control', 'placeholder'=>'Phone Number')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('lead_phone_alt', 'Alternative Phone Number') }}
                                        {{ Form::text('lead_phone_alt', Input::old('lead_phone_alt'), array('class' => 'form-control', 'placeholder'=>'Alternative Phone')) }}
                                    </div>
                                    <div class="form-group">
                                        {{ Form::label('lead_message', 'Message') }}
                                        <textarea name="lead_message" class="form-control lead_message" placeholder="Message" rows="10"></textarea>
                                    </div>
                        ...
                        <div class="panel box box-danger">
                            <div class="box-header with-border">
                                <h4 class="box-title">
                                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" onclick="changeAcc('sms');" class="collapsed" aria-expanded="false" data-active="false">
                                        Copy & Paste Lead
                                    </a>
                                </h4>
                            </div>
                            <div id="collapseTwo" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
                                <div class="box-body">
                                    <div class="form-group">
                                        {{ Form::label('lead_message_type', 'Message') }}
                                        <textarea name="lead_message_type" class="form-control lead_message_type" placeholder="Message" rows="10"></textarea>
                                        <p class="help-block">To add a line break use ** in your message</p>
                                    </div>

                                    <div class="input-group input-group-sm">
                                        <select class="form-control" style="-webkit-border-radius: 0px;" id="premessageSelect2">

                                        </select>
                                        <span class="input-group-btn">
                                          <button type="button" class="btn btn-success btn-flat addPreMessage2">Add</button>
                                        </span>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="form-group">
                    ...
                    {{ Form::text('lead_area', Input::old('lead_area'), array('class' => 'form-control area', 'placeholder'=>'Location', 'id' => 'autocomplete')) }}
                </div>
                ...
                <table class="table table-bordered contractorsTable table-condensed" id="example2">
                    <thead>
...
$('.loadingApply').css('opacity', 0);
    ...
    $.each(resultIndustry, function(key, value){
        $('.lead_industry').append($('<option value="'+key+'+'+key+'"></option>').html(key));
		$.each(resultIndustry[key], function(key0, value0){
			$('.lead_industry').append($('<option value="'+key+'+'+value0+'"></option>').html("--"+value0));
		});
    });
    ...
    $.each(resultPreMessage, function(keyPre, valuePre){
        ...
        $('#premessageSelect2').append($('<option value="'+keyPre+'"></option>').html(resultPreMessage[keyPre].name));
    });
    ...
    $('.addPreMessage2').on('click', function(){
        var messagekey = $('#premessageSelect2').val();
        console.log(messagekey);
        var messageVal = $('.lead_message_type').val();
        var message = resultPreMessage[messagekey].message;
        $('.lead_message_type').html(""+messageVal+" "+message);
    });
...
function changeAcc(item) {
    ...
    $('.searchForContractor').on('click', function() {
